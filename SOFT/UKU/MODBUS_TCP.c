...
//o_1_s
			...
			for(io=0;io<sendbuf[8];++io) {
				...
				if(adrr_reg<MODBUS_FUNC_3_LENGTH) sendbuf[9+io]=*reg_func3[adrr_reg];
				else sendbuf[9+io]=0;
			}	 
...
//o_1_s
			...
			for(io=0;io<sendbuf[8];++io) {
				unsigned long adrr_reg;
				adrr_reg=start_adr_tcp_mb+io;
				if(adrr_reg<MODBUS_FUNC_4_LENGTH) sendbuf[9+io]=*reg_func4[adrr_reg];
				else sendbuf[9+io]=0;
			}	 
          	tcp_send (socket_tcp, sendbuf, (start_num_tcp_mb+9));
//o_1_e
...
*/  //o_1			
			...
			{
//o_1_s   обработку функции 6 заменить полностью,
				U8 *sendbuf;
